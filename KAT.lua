-- =========================================================================
-- EMLOXA WARE: KNIFE ABILITY TEST (KAT) V13 DISTANCE CORE (HUB FIX - PREMIUM)
-- =========================================================================
local GameModule = {}
...
function GameModule:Init(Window)
    ...
    FOVCircle.Thickness = 1.5; FOVCircle.NumSides = 50; FOVCircle.Filled = false
    FOVCircle.Transparency = 1; FOVCircle.Color = Color3.fromRGB(255, 255, 255)
    ...
    -- COMBAT TAB
    CombatTab:CreatePremiumToggle("RageBot (360°)", function(v) Settings.Combat.RageBot = v end)
    ...
    CombatTab:CreatePremiumToggle("Silent Aim (Micro Flick)", function(v) Settings.Combat.SilentAim = v end)
    
    CombatTab:CreatePremiumToggle("TriggerBot", function(v) Settings.Combat.TriggerBot = v end)
    ...
    -- MISC TAB
    MiscTab:CreatePremiumToggle("SpinBot", function(v) Settings.Misc.SpinBot = v end)
                ...
                if myRoot and targetRoot then
                    ...
                    if dist <= maxDistance then
                        if Settings.Combat.WallCheck then
                            if IsVisible(plr.Character.Head) then target = plr; maxDistance = dist end
                        else target = plr; maxDistance = dist end
                    end
                end
            end
        end
            ...
            if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("Head") and plr.Character.Humanoid.Health > 0 then
                ...
                if onScreen then
                    ...
                    if dist < shortest then
                        if IsVisible(plr.Character.Head) then shortest = dist; target = plr end
                    end
                end
            end
        end
    ...
    table.insert(Connections, RunService.RenderStepped:Connect(function()
        ...
        if Settings.Combat.Aimbot and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
            ...
            if target then Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position, target.Character.Head.Position), 1/Settings.Combat.Smoothness) end
        end
            ...
            for _, plr in pairs(Players:GetPlayers()) do
                if plr ~= LocalPlayer and plr.Character then
                    ...
                    if hum and hum.Health > 0 and head then
                        ...
                        if plr.Character:FindFirstChild("EMLOXA_ESP") then plr.Character.EMLOXA_ESP:Destroy() end
                    end
                end
            end
        else
            for _, plr in pairs(Players:GetPlayers()) do
                if plr.Character and plr.Character:FindFirstChild("EMLOXA_ESP") then plr.Character.EMLOXA_ESP:Destroy() end
            end
        end
