...
-- ========================================
-- KİMLİK GİZLEME (IDENTITY HIDER)
-- Workspace, CoreGui, PlayerGui ve gethui() dahil her yeri kontrol eder.
-- ========================================
local identityHidingLoop = nil

local function getIdentityTargets()
local targets = {}
local core = game:GetService("CoreGui")
if core then table.insert(targets, core) end

local pg = LocalPlayer:FindFirstChild("PlayerGui")
if pg then table.insert(targets, pg) end

-- gethui (gelişmiş executorlar için)
local hui = (gethui and gethui()) or nil
if hui then table.insert(targets, hui) end

-- Workspace içindeki tüm GUI'ler (BillboardGui, SurfaceGui, ScreenGui vb.)
pcall(function()
...
local function processObject(obj, enabled, originalName, targetName, targetId)
-- Metin kontrolleri (TextLabel, TextButton, TextBox)
if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
if obj.Text and type(obj.Text) == "string" then
if string.find(obj.Text, originalName) or string.find(obj.Text, targetName) then
obj.Text = enabled and targetName or originalName
end
end
-- Resim kontrolleri (profil fotoğrafı)
elseif obj:IsA("ImageLabel") or obj:IsA("ImageButton") then
local image = obj.Image
if image and type(image) == "string" then
local hasUserId = string.find(image, "UserId=" .. targetId) or
string.find(image, "&id=" .. targetId) or
string.find(image, "userid=" .. targetId) or
string.find(image, "&userId=" .. targetId)
if hasUserId then
obj.ImageTransparency = enabled and 1 or 0
end
end
end
...
processObject(child, enabled, originalName, targetName, targetId)
end
end

local function applyIdentityHiding(enabled)
local originalName = LocalPlayer.DisplayName or LocalPlayer.Name
local targetName = "EMLOXAWARE USER"
local targetId = LocalPlayer.UserId

for _, root in ipairs(getIdentityTargets()) do
pcall(processObject, root, enabled, originalName, targetName, targetId)
end
end

local function startIdentityHiding(enabled)
if identityHidingLoop then
task.cancel(identityHidingLoop)
identityHidingLoop = nil
end
if enabled then
identityHidingLoop = task.spawn(function()
while true do
applyIdentityHiding(true)
task.wait(1) -- 1 saniyede bir tazele
end
end)
else
applyIdentityHiding(false)
end
end
-- ========================================
...
-- 🔥 Kimlik Gizleme Toggle (En Üstte)
MenuTab:CreateToggle("Kimlik Gizleme", function(state)
startIdentityHiding(state)
end)
