...
CurrentTheme = theme
for , entry in ipairs(ThemeObjects) do
...
-- ══════════════════════════════════════
-- IDENTITY HIDER SYSTEM (ULTRA-FAST OPTIMIZED)
-- ══════════════════════════════════════
local identityHiderStateFile = BaseConfigFolder .. "/identity_hider_state.json"
local identityHiderEnabled = false

-- Dedicated tables for ultra-fast direct modification
local knownTextInstances = {} -- instance -> true
local knownImageInstances = {} -- instance -> true
...
-- Process a single instance and return:
-- "text" -> needs constant text replacement
-- "image" -> needs constant image removal
-- nil -> not relevant
local function ClassifyInstance(instance, localPlayerName, localDisplayName, localUserIdStr)
if not instance or not instance.Parent then return nil end
if HubGui and instance:IsDescendantOf(HubGui) then return nil end -- skip our own UI

-- Text elements
if instance:IsA("TextLabel") or instance:IsA("TextButton") or instance:IsA("TextBox") then
local original = instance.Text
if original and original ~= "" then
-- Check if contains player's name or display name (fast check)
if string.find(original, localPlayerName, 1, true) or string.find(original, localDisplayName, 1, true) then
return "text"
end
end
end

-- Image elements (avatar hiding) — only if it's the local player's avatar
if instance:IsA("ImageLabel") or instance:IsA("ImageButton") then
local img = instance.Image
if img and img ~= "" then
-- Only hide if it contains the local player's UserId
if string.find(img, localUserIdStr, 1, true) then
return "image"
end
end
end
...
-- Fast scan: directly set known instances without any checks (super fast)
local function FastScan()
local replacementText = "EMLOXAWARE USER"
for instance in pairs(knownTextInstances) do
if instance.Parent then
instance.Text = replacementText
else
knownTextInstances[instance] = nil
end
end
for instance in pairs(knownImageInstances) do
if instance.Parent then
instance.Image = ""
instance.ImageTransparency = 1
else
knownImageInstances[instance] = nil
end
end
end

-- Full scan: iterate all descendants to discover new relevant instances
...
local localDisplay = LocalPlayer.DisplayName
...
if obj and obj.Parent then
local classification = ClassifyInstance(obj, localName, localDisplay, localUserId)
if classification == "text" then
knownTextInstances[obj] = true
elseif classification == "image" then
knownImageInstances[obj] = true
end
end
end
end
...
FullScan()
task.wait(0.5) -- discover new instances every 0.5 seconds
end
end
...
for k in pairs(knownTextInstances) do knownTextInstances[k] = nil end
for k in pairs(knownImageInstances) do knownImageInstances[k] = nil end
end
