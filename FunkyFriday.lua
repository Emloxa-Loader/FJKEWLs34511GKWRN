...
function GameModule:Init(Window)
    ...
    RunService.RenderStepped:Connect(function(deltaTime)
                ...
                if notesFolder then
                    for _, note in pairs(notesFolder:GetChildren()) do
                        if note:IsA("GuiObject") then
                            ...
                            if AutoplayMethod == "Rapid checks" then shouldHit = (dist <= 3) 
                            elseif AutoplayMethod == "Calculate" then shouldHit = (dist <= math.max(2, frameTravel / 1.5))
                            elseif AutoplayMethod == "Hybrid" then shouldHit = (dist <= math.max(3, frameTravel / 1.2)) end
        ...
        if not anyNoteSeenThisFrame and (tick() - LastNoteSeenTime > 2.5) then
            ...
            CountedNotes = {}
            LastYPositions = {}
            for holdNote, key in pairs(ActiveHolds) do VirtualInputManager:SendKeyEvent(false, key, false, game) end
            ActiveHolds = {}
            LastNoteSeenTime = tick()
        end
    end)
